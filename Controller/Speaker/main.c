...
static void pwm_init(void)
{
    ...
    // --- GPIOB Setup ---
    // Enable GPIOB Clock (You already have this, but keeping for context)
    RCC->IOPENR |= (1U << 1U); 
    
    // PB1 and PB0 setup: Set to Input Mode (00)
    // PB5 setup: Set to Output Mode (01)
    // Clear bits for 0, 1, and 
    
	// In pwm_init(), replace the PB5 setup with:
	GPIOA->MODER &= ~(3U << 28U);   // clear PA14
	GPIOA->MODER |=  (1U << 28U);   // PA14 = output
